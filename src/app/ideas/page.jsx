...
const IdeasPage = () => {
  ...
  {/* FILTER SECTION */}
  <div className="flex flex-col items-center justify-center text-center mb-10 bg-gray-200 p-4 sm:p-6 rounded-xl">

    {/* Heading */}
    <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-2 text-lime-800">
      Discover Ideas
    </h2>
    ...
    <div className="flex flex-col lg:flex-row items-center gap-4 w-full justify-center">
      ...
      <input
        ...
        className="border p-3 rounded-full w-full lg:w-1/2 bg-gray-300 px-6 focus:outline-none focus:ring-2 focus:ring-lime-400"
      />

      <select
        value={category}
        onChange={(e) => setCategory(e.target.value)}
        className="bg-lime-100 border border-lime-100 p-3 sm:p-4 rounded w-full lg:w-52"
      >
      ...
      {["All", "Health", "Tech", "Agriculture", "AI","Education"].map((item) => {
        ...
        return (
          <button
            ...
            className={`px-3 sm:px-4 py-2 rounded-full border transition font-medium text-sm sm:text-base
              ${
                isActive
          ...
          <div className="p-4 sm:p-6">
            ...
            <div className="flex flex-wrap gap-2 mt-3 mb-6">
              {idea.tags?.map((tag, index) => (
                <span
                  key={index}
                  className="bg-lime-200 text-green-700 px-3 py-1 rounded-full text-xs sm:text-sm"
                >
                  #{tag.toLowerCase()}
                </span>
              ))}
            ...
            <Link
              href={`/ideas/${idea._id}`}
              className="mt-2 flex items-center justify-center gap-2 bg-lime-500 hover:bg-lime-600 text-white py-3 rounded-xl transition"
            >
